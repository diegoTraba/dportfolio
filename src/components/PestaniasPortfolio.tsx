...
export const PestaniasPortfolio = ({ pestanias }: PestaniasPortfolioPropiedades) => {
  ...
  const getActiveTab = () => {
    // Normalizar el pathname (eliminar trailing slashes)
    ...
    // Si no hay ninguna coincidencia, usar la primera pestaña
  ...
  return (
    <div className="border-[var(--colorTerciario)]">
          ...
          <Link
            ...
            className={`${
              activeTab === pestania.id
                ? 'border-[var(--colorTerciario)] text-[var(--colorTerciario)]'
                : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-300'
            } whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm flex items-center space-x-2 transition-colors duration-200`}
          >
            {pestania.icon && <span>{pestania.icon}</span>}
